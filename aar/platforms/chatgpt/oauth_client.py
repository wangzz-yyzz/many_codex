...
class OAuthClient:
    ...
    @staticmethod
    def _parse_retry_after_seconds(retry_after_value) -> int:
        """解析 Retry-After 秒数，解析失败返回 0。"""
        try:
            text = str(retry_after_value or "").strip()
            if not text:
                return 0
    ...
    def _compute_rate_limit_backoff(self, attempt_idx: int, retry_after_seconds: int = 0) -> float:
        """计算限流退避时间，优先使用 Retry-After，否则指数退避并加抖动。"""
        base_schedule = (8, 20, 45, 75)
        base_delay = base_schedule[min(max(0, int(attempt_idx or 0)), len(base_schedule) - 1)]
        jitter = random.uniform(0.5, 2.0)
        if retry_after_seconds > 0:
            return float(min(180, retry_after_seconds) + jitter)
        return float(base_delay + jitter)

    @staticmethod
    def _extract_retry_after_from_error(error_message: str) -> int:
        """从错误文案中提取 retry_after=XXs。"""
        text = str(error_message or "")
        match = re.search(r"retry_after\s*=\s*(\d+)", text, re.IGNORECASE)
        if not match:
            return 0
        try:
    ...
    def _bootstrap_oauth_session(
        ...
        try:
            ...
            if r.status_code == 429:
                ...
                self._set_error(
                    f"/oauth/authorize 命中 429 限流"
                    + (f", retry_after={retry_after}s" if retry_after > 0 else "")
        ...
        try:
            ...
            if r2.status_code == 429:
                retry_after = self._parse_retry_after_seconds(
                    (getattr(r2, "headers", {}) or {}).get("Retry-After", "")
                )
                request_id = (getattr(r2, "headers", {}) or {}).get("x-request-id", "")
                self._set_error(
                    f"/api/oauth/oauth2/auth 命中 429 限流"
                    + (f", retry_after={retry_after}s" if retry_after > 0 else "")
                    + (f", request_id={request_id}" if request_id else "")
                )
                return ""
        ...
        *,
        ...
        try:
            max_rate_limit_retries = int(
                self.config.get("chatgpt_oauth_429_max_retries", 2) or 2
            )
        except Exception:
            max_rate_limit_retries = 2
        max_rate_limit_retries = max(0, min(max_rate_limit_retries, 6))
        invalid_auth_step_retried = False
        ...
        for attempt in range(max_rate_limit_retries + 1):
            try:
                ...
                if (
                    ...
                    and not invalid_auth_step_retried
                ):
                    invalid_auth_step_retried = True
        ...
        *,
        ...
        try:
            bootstrap_max_retries = int(
                self.config.get("chatgpt_oauth_bootstrap_max_retries", 1) or 1
            )
        except Exception:
            bootstrap_max_retries = 1
        bootstrap_max_retries = max(0, min(bootstrap_max_retries, 5))
        authorize_final_url = ""
        for attempt in range(bootstrap_max_retries + 1):
            ...
            if attempt >= bootstrap_max_retries:
                break
            retry_after_hint = self._extract_retry_after_from_error(self.last_error)
            wait_seconds = self._compute_rate_limit_backoff(attempt, retry_after_hint)
            self._log(
                f"Bootstrap 未成功，准备重试 {attempt + 1}/{bootstrap_max_retries + 1}，"
