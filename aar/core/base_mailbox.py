...
class LuckMailMailbox(BaseMailbox):
    ...
    def _email_matches_domain(self, email: str) -> bool:
        ...
        if "@" not in email_value:
            return False
        domain = str(self._domain or "").strip().lower()
        if not domain:
            return True
        return email_value.rsplit("@", 1)[-1] == domain

    def _find_reusable_purchase(self):
        domain_text = self._domain or "-"
        self._log(
            f"[LuckMail] 优先尝试复用已购邮箱 "
            f"(project_code={self._project_code}, domain={domain_text})"
        )
        ...
        def _pick_item(items):
            for item in items or []:
                ...
                if not email or not token:
                    continue
                if int(getattr(item, "user_disabled", 0) or 0) == 1:
                    continue
                if not self._email_matches_domain(email):
                    continue
                return item
            return None

        try:
            api_items = self._client.user.api_get_purchases(count=10)
            picked = _pick_item(api_items)
            if picked:
                return picked
        except Exception as e:
            self._log(f"[LuckMail] 复用已购邮箱(api_get_purchases)失败，转普通列表: {e}")

        try:
            page = self._client.user.get_purchases(
                page=1,
                page_size=100,
                user_disabled=0,
            )
            picked = _pick_item(getattr(page, "list", []) or [])
            if picked:
                return picked
        except Exception as e:
            self._log(f"[LuckMail] 复用已购邮箱(get_purchases)失败: {e}")
        return None
    ...
    def get_email(self) -> MailboxAccount:
        ...
        if self._use_purchase_mode():
            self._log(
                f"[LuckMail] 分支: ChatGPT + LuckMail -> 已购优先，购买兜底 "
                f"(project_code={self._project_code}, email_type={self._email_type or '-'}, domain={self._domain or '-'})"
            )
            reusable = self._find_reusable_purchase()
            if reusable:
                email = str(getattr(reusable, "email_address", "") or "").strip()
                token = str(getattr(reusable, "token", "") or "").strip()
                self._email = email
                self._token = token
                self._log(f"[LuckMail] 复用已购邮箱: {email}")
                return MailboxAccount(
                    email=email,
                    account_id=token,
                    extra={
                        ...
                        "purchase_id": getattr(reusable, "id", 0),
                    },
                )

            self._log("[LuckMail] 未命中可复用已购邮箱，开始购买新邮箱...")
            try:
    ...
    def wait_for_code(
        ...
        **kwargs,
        ...
        # LuckMail 收码统一限制 30 秒，超时立即失败，避免长时间阻塞流程。
        try:
            raw_timeout = int(timeout or 0)
        except Exception:
            raw_timeout = 0
        effective_timeout = max(1, min(raw_timeout if raw_timeout > 0 else 120, 30))
        if raw_timeout != effective_timeout:
            self._log(
                f"[LuckMail] 收码超时已限制为 {effective_timeout}s "
                f"(请求值: {raw_timeout if raw_timeout > 0 else timeout})"
            )
        ...
        if not self._use_purchase_mode(account):
            ...
            self._cancel_order_silently(order_no)
            raise TimeoutError(
                f"LuckMail 等待验证码超时 ({effective_timeout}s)，最终状态: {last_status}"
            )
        ...
        try:
            alive_result = self._client.user.check_token_alive(token)
            if getattr(alive_result, "alive", False):
                self._log(
                    f"[LuckMail] Token 测活通过: status={getattr(alive_result, 'status', '-')}, "
                    f"mail_count={getattr(alive_result, 'mail_count', 0)}"
                )
            else:
                self._log(
                    f"[LuckMail] Token 测活失败: status={getattr(alive_result, 'status', '-')}, "
                    f"message={getattr(alive_result, 'message', '')}"
                )
        except Exception as e:
            self._log(f"[LuckMail] Token 测活异常，继续尝试收码: {e}")
        ...
        def poll_once() -> Optional[str]:
            ...
            try:
                token_code = self._client.user.get_token_code(token)
            except Exception as e:
                raise TimeoutError(f"LuckMail 查询 token/code 失败: {e}") from e

            has_new_mail = bool(getattr(token_code, "has_new_mail", False))
            quick_code = str(getattr(token_code, "verification_code", "") or "").strip()
            quick_mail = getattr(token_code, "mail", None) or {}
            quick_message_id = str(
                quick_mail.get("message_id")
                or quick_mail.get("id")
                or quick_mail.get("mail_id")
                or ""
            ).strip()
            quick_ts = _normalize_timestamp(
                quick_mail.get("received_at")
                or quick_mail.get("date")
                or quick_mail.get("timestamp")
                or quick_mail.get("created_at")
            )
            should_scan_mails = bool(has_new_mail)
            quick_code_rejected = False
            if quick_code:
                if not has_new_mail:
                    self._log(
                        f"[LuckMail] 跳过历史验证码(token/code): code={quick_code} has_new_mail=false"
                    )
                    should_scan_mails = True
                    quick_code_rejected = True
                if quick_message_id and quick_message_id in seen_message_ids:
                    self._log(
                        f"[LuckMail] 跳过历史验证码(token/code): message_id={quick_message_id}"
                    )
                    should_scan_mails = True
                    quick_code_rejected = True
                if otp_cutoff and quick_ts and quick_ts < otp_cutoff:
                    self._log(
                        f"[LuckMail] 跳过旧验证码(token/code): code={quick_code} "
                        f"mail_ts={quick_ts:.0f} cutoff={otp_cutoff:.0f}"
                    )
                    should_scan_mails = True
                    quick_code_rejected = True
                if should_scan_mails and (not quick_code_rejected):
                    if quick_code in exclude_codes:
                        self._log(f"[LuckMail] 跳过已使用验证码 code={quick_code}")
                    else:
                        if quick_message_id:
                            seen_message_ids.add(quick_message_id)
                        self._log(f"[LuckMail] 收到验证码(token/code): {quick_code}")
                        return quick_code

            # token/code 返回有新邮件但未直接给出验证码时，再走邮件列表兜底解析。
            if not should_scan_mails:
                self._log("[LuckMail] 轮询中... 新邮件: 否")
                return None
            ...
            for mail in mail_list.mails:
                ...
                message_ts = _normalize_timestamp(getattr(mail, "received_at", ""))
                body = " ".join(
                    [
                        str(mail.subject or ""),
                        str(mail.body or ""),
                ...
                # Token 邮件列表接口通常仅返回摘要，正文为空时回查详情接口。
                if not mail.body and not mail.html_body and message_id:
                    try:
                        detail = self._client.user.get_token_mail_detail(token, message_id)
                        if message_ts is None:
                            message_ts = _normalize_timestamp(
                                getattr(detail, "received_at", "")
                            )
                        body = " ".join(
                            [
                                str(detail.subject or ""),
                                str(detail.body_text or ""),
                                str(detail.body_html or ""),
                            ]
                        )
                    except Exception:
                        pass
                if otp_cutoff and message_ts and message_ts < otp_cutoff:
                    self._log(
                        f"[LuckMail] 跳过旧邮件验证码: message_id={message_id or '-'} "
                        f"mail_ts={message_ts:.0f} cutoff={otp_cutoff:.0f}"
                    )
                    continue
                code = self._safe_extract(body, code_pattern)
                if code and code in exclude_codes:
        ...
        return self._run_polling_wait(
            ...
            poll_once=poll_once,
            timeout_message=(
                f"LuckMail 等待验证码超时 ({effective_timeout}s)，最终状态: "
