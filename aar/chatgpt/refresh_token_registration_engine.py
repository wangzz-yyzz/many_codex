...
class RefreshTokenRegistrationEngine:
    ...
    def _is_rate_limited_error(message: str) -> bool:
        text = str(message or "").lower()
        markers = (" 429", "429 ", "rate limit", "too many requests", "限流")
        return any(marker in text for marker in markers)
    ...
    def run(self) -> RegistrationResult:
        ...
        try:
            ...
            if not tokens:
                ...
                if use_continued_session and self._is_rate_limited_error(last_error):
                    retry_after_hint = self._extract_retry_after_seconds(last_error)
                    oauth_429_cooldown_seconds = self._read_int_config(
                        "chatgpt_oauth_429_cooldown_seconds",
                        fallback_keys=("chatgpt_oauth_cooldown_seconds",),
                        default=45,
                        minimum=5,
                        maximum=600,
                    )
                    cooldown_seconds = max(oauth_429_cooldown_seconds, retry_after_hint)
                    self._log(
                        "OAuth 接续阶段命中 429 限流，"
                        f"冷却 {cooldown_seconds}s 后切换全新会话重试一次...",
                        "warning",
                    )
                    time.sleep(cooldown_seconds)
                    fallback_oauth_client = self._build_oauth_client()
                    fallback_oauth_client.config.setdefault(
                        "chatgpt_oauth_otp_wait_seconds",
                        register_otp_wait_seconds,
                    )
                    fallback_oauth_client.config.setdefault(
                    ...
                    tokens = fallback_oauth_client.login_and_get_tokens(
                        result.email,
                        self.password,
                        device_id="",
                        user_agent=getattr(register_client, "ua", None),
                        ...
                        last_name=last_name,
                        birthdate=birthdate,
                        login_source="post_register_429_fallback",
                    )
                    if tokens:
                        oauth_client = fallback_oauth_client
                    else:
                        last_error = (
                            fallback_oauth_client.last_error
                            or last_error
                            or "OAuth 登录状态机失败"
                        )
                if "add_phone" in last_error:
                    self._log(
                        "OAuth add_phone 阻断，按策略不重试并直接失败",
                        "warning",
                    )
                    result.error_message = last_error
                    return result
